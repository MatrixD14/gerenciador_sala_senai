...
class Tabelas
{
    ...
    {
        file_put_contents("debug_filtro.txt", print_r($_POST, true), FILE_APPEND);
        ...
        foreach ($config['colunas'] as $nomeCol => $prop) {
            ...
            if ($valorFiltro !== null && $valorFiltro !== '') {
                ...
                $prefixo = $tabelaPrincipal;
                $colReal = self::getCleanColumnName($nomeCol);
                if (isset($prop['relation']['tabela'])) {
                    $prefixo = $prop['relation']['tabela'];
                    $colReal = $prop['relation']['coluna'];
                }
                if (($prop['type'] ?? '') === 'select' || $nomeCol === 'id') {
                    $condicoes[] = "$prefixo.$colReal = '$valorEsc'";
                } else {
                    $condicoes[] = "$prefixo.$colReal LIKE '%$valorEsc%'";
                }
            }
        ...
        if ($searchTerm) {
            ...
            if (!empty($config["especifico"])) {
                foreach ($config["especifico"] as $c) {
                    $partes = explode(' as ', $c);
                    $campoReal = trim($partes[0]);

                    if (strpos($campoReal, '(') === false) {
                        $filtros[] = "$campoReal LIKE '%$termoOriginal%'";
                    }
                }
            } else {
                foreach (array_keys($config["colunas"]) as $c) {
                    $colFiltro = self::getCleanColumnName($c);
                    if (strpos($colFiltro, '.') === false) {
                        $colFiltro = "$tabelaPrincipal.$colFiltro";
                    }
