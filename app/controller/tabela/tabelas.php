...
class Tabelas
{
    ...
    public static function geraBodyTabela2($slug, $UserLogin = null): string
    {
        ...
        $filtros = array_filter($_POST, function ($v, $k) {
            return $v !== '' && $v !== null && $k !== 'search';
        ...
        if ($listDate && $listDate->num_rows >= $limit) {
        ...
        if ($listDate->num_rows < $limit) {
            $html .= "<tr class='sentinel'
                        data-slug='$slug'
                        data-end='true'>
                        <td colspan='100%'>Fim dos resultados</td>
                    </tr>";
        }
    ...
    private static function searchTabela($slug, $config, $UserLogin, $limit): string
    {
        ...
        if ($lastId && empty($searchTerm)) $condicoes[] = "$tabelaPrincipal.id > $lastId";
        ...
        if ($searchTerm) {
            ...
            foreach ($colunasParaFiltro as $c) {
                ...
                $partes = explode(' as ', strtolower($c));
                $alias = trim(end($partes));

                if ($alias === strtolower($c)) {
                    $alias = self::getCleanColumnName($c);
                }
                if (strpos($campoCru, '.') === false) {
                    $campoCru = "$tabelaPrincipal.$campoCru";
                }
                // $nomeColuna = self::getCleanColumnName($c);
                if ($colunasVisiveis !== null && !in_array($alias, $colunasVisiveis)) continue;
                error_log("COLUNA: " . $alias);
                error_log("VISIVEIS: " . print_r($colunasVisiveis, true));
                $tipoColuna = $config["colunas"][$alias]['type'] ?? $config["colunas"][self::getCleanColumnName($c)]['type']
                    ?? null;;
                if ($tipoColuna === 'date') {
        ...
        $query = "SELECT $select FROM $tabelaPrincipal $joins $where $order LIMIT $limit";
