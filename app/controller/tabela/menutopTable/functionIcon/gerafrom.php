...
class gerarFromDinamico
{
    ...
    public static function geraFrom($table, $id = null, $data = null, $user = null, $readonlyActive = false): string
    {
        ...
        if ($id !== null) {
            ...
            if (isset($dados['dia'])) {
                $dataAgendamento = $dados['dia'];
            }
            if (($dataAgendamento && $dataAgendamento < $hoje) || $readonlyActive === true) {
        ...
        foreach ($coluneSelect as $coluna => $configColuna) {
            ...
            $valorOriginalDoBanco = $dados[$campoBanco] ?? '';
            $valorBanco = $valorOriginalDoBanco;
            if ($campoBanco === 'idUser' && $userPrivilegio === 'normal' && $id === null) {
                $valorBanco = $userId;
                $tipo = 'readonly_user';
            }
            if (empty($valorBanco) && $tipo === 'date' && $dataAgendamento)
                $valorBanco = $dataAgendamento;
            ...
            if ($tipo === 'readonly_user') {
                ...
                $valorFormatado = $valorEscapado;
                if (!empty($valorEscapado) && strpos($valorEscapado, '/') !== false) {
                    $partes = explode('/', $valorEscapado);
                    $valorFormatado = "{$partes[2]}-{$partes[1]}-{$partes[0]}";
                }
                $html .= "<input type='date' name='$coluna' id='$coluna' class='input-dados' value='$valorFormatado' placeholder='dd/mm/aaaa' $readonlyAttr $minAttr>";
                ...
                if ($isRegistroBloqueado) {
                    ...
                    if (!empty($configColuna['options'])) {
                        foreach ($configColuna['options'] as $labelOpt => $regra) {
                            if (is_array($regra)) {
                                $optValue = $labelOpt;
                                $min = $regra['min'] ?? 0;
                                $max = $regra['max'] ?? 24;
                                $foraDoHorario = ($isHoje && ($horaAtual < $min || $horaAtual >= $max));
                                $disabled = $foraDoHorario ? "disabled" : "";
                                $labelExibicao = $foraDoHorario ? "$labelOpt *" : $labelOpt;
                            } else {
                                $optValue = $regra;
                                $disabled = "";
                                $labelExibicao = $regra;
                            ...
                            $selected = ($optValue == $valorBanco) ? "selected" : "";
                            $html .= "<option value='$optValue' $selected $disabled>$labelExibicao</option>";
                        }
                    }
