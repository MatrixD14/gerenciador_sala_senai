...
class FormEngine
{
    ...
    public function render(): string
    {
        ...
        foreach ($this->config['colunas'] as $name => $colConfig) {
            ...
            $html .= $this->renderRow($name, $colConfig, $value);
        }
        return $html;
    }

    private function renderRow($name, $col, $val): string
    {
        $label = ($col['type'] !== 'hidden') ? "<label for='$name'>" . ucfirst($name) . "</label><br>" : "";
        $field = FormRenderer::renderField($name, $col, $val, $this->isLocked, $this->config['colunas']);
