...
function executarBusca(input, scrollArea, term) {
    ...
    scrollArea.innerHTML = '<div style="padding:10px; text-align:center;">Buscando...</div>';

    fetch('/buscaList', { method: 'POST', body: fd })
        .then((r) => r.text())
        .then((html) => {
            if (!html.trim()) {
                scrollArea.innerHTML = '<div style="padding:10px; color: #999;">Nenhum resultado encontrado.</div>';
            } else {
                scrollArea.innerHTML = html;
            }
        })
        .catch(() => {
            scrollArea.innerHTML = '<div style="padding:10px; color: red;">Erro na busca.</div>';
        });
}
