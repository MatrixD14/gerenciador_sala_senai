  ...
  static construirLinhas(json) {
    ...
    if (config.especifico && config.especifico.length > 0) {
      ...
      if (config.colunas_visiveis && Array.isArray(config.colunas_visiveis)) {
        colunasParaExibir = config.colunas_visiveis.filter((alias) =>
          aliasMap.includes(alias),
        );
      } else {
      ...
      if (config.colunas_visiveis && Array.isArray(config.colunas_visiveis)) {
        colunasParaExibir = config.colunas_visiveis.filter((col) =>
          todasColunas.includes(col),
        );
      } else {
    ...
    let colunasParaRenderizar =
      config.colunas_visiveis || Object.keys(config.colunas);
