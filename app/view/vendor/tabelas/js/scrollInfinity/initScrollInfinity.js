...
import { criarSentinelas, configurarObserver, salvarFiltrosNoDOM } from './domScroll.js';
...
window.aplicarFiltros = function (dadosFiltro) {
    const container = document.getElementById('carregaTabela');
    if (!container) {
        console.error('Container #carregaTabela não encontrado. Recarregando página...');
        location.reload();
        return;
    }
...
window.initTabela = async function (slug, searchTerm = '', filtrosIniciais = {}) {
    if (
