...
document.addEventListener("contentUpdated", function () {
  ...
  timeoutContentUpdated = setTimeout(() => {
    ...
    if (
      window.tabelaState &&
      window.tabelaState.slug === slug &&
      window.tabelaState.blocosCarregados.has(0)
    ) {
      return;
    }
    ultimoSlugProcessado = slug;
    const currentSearch = window.tabelaState ? window.tabelaState.search : "";
    const filtros = tbody.dataset.filtros
      ? JSON.parse(tbody.dataset.filtros)
      : {};
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  let filtrosIniciais = {};
  try {
    filtrosIniciais = JSON.parse(
      container.getAttribute("data-filtros") || "{}",
    );
